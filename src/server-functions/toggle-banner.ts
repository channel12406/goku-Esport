...
export const toggleBanner = createServerFn({ method: "POST" })
  .validator((input: { idToken: string; bannerId: string; active: boolean }) => {
  ...
  .handler(async ({ data }) => {
    ...
    await updateDoc(doc(serverDb, "banners", data.bannerId), {
      active: data.active,
    });
    return { success: true };
  });
